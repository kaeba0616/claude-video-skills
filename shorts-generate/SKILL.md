...
**장면이 물리적으로 이어져야 하는 소재면 프레임 체이닝을 기본으로 쓴다** (5-0 참조).
같은 잔·같은 손·같은 장소에서 연속 동작이 진행되는 영상(요리, 드로잉, 만들기 과정 등)은
스타일 문구 반복만으로는 부족하다 — text-to-video 는 클립마다 소품·구도를 새로 뽑기
때문에 컷이 튄다. 반대로 장면마다 장소·피사체가 바뀌는 영상(여행, 몽타주)은 체이닝이
필요 없고 병렬 생성이 빠르다.
...
**5-0. 생성 모드 선택 — 세 가지 기술**

| 기술 | 플래그 | 쓰는 경우 | 모델 |
|---|---|---|---|
| text-to-video | (없음) | 장면 간 연결이 필요 없을 때 (병렬 생성 가능) | 전부 |
| image-to-video | `--image <png>` | **프레임 체이닝** — 이전 클립에서 이어질 때 | 전부 (lite 포함, 실측) |
| reference-to-video | `--reference-image <사진>` | 실제 제품·피사체 외형 유지 (광고) | fast 이상 |

**프레임 체이닝 (연속 동작 영상의 기본)**: 클립 N의 마지막 프레임을 뽑아 클립 N+1의
첫 프레임으로 넣는다. 잔·소품·구도가 물리적으로 이어진다 (실측: 라떼 에칭 3클립에서
같은 잔·같은 테이블 유지 확인).

```bash
# 클립 1은 text-to-video로 생성 후, 마지막 프레임 추출
ffmpeg -v error -sseof -0.2 -i clips/scene_01.mp4 -frames:v 1 -update 1 frames/scene_01_last.png

# 클립 2는 그 프레임에서 시작 (해상도가 이미 목표 화면비라 크롭 불필요)
python3 ~/.claude/skills/shorts-generate/scripts/veo_generate.py \
  --prompt-file prompts/scene_02.txt --out clips/scene_02.mp4 \
  --image frames/scene_01_last.png \
